...
int sequence_read_frame(Sequence *seq, AVFrame *frame,
  ...
  int ret;
  // If VideoContext was used by another clip, and is now out of bounds of
  // current clip. Reset seek
  ...
  if (ret < 0) {
    ...
    if (next == NULL) {
      ...
      if (ret < 0) {
        fprintf(stderr, "sequence_read_frame() error: Failed to seek to the "
                        "start of sequence\n");
        return ret;
      }
      return -1;
