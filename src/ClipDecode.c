    ...
    if (vid_ctx->last_decoder_packet_stream == DEC_STREAM_VIDEO) {
      *frame_type = AVMEDIA_TYPE_VIDEO;
      ...
      if (ret == 0) {
        ...
        // output is not available in this state - user must try to send new
        // input
      ...
      *frame_type = AVMEDIA_TYPE_AUDIO;
      ...
      if (ret == 0) {
        // success, frame was returned from decoder
        int64_t seek_pts =
            cov_video_to_audio_pts(clip->vid_ctx, clip->vid_ctx->seek_pts);
...
int clip_send_packet(Clip *clip) {
  ...
  if (pkt.stream_index == vid_ctx->video_stream_idx) {
    ...
    printf("packet is not video or audio stream. (we should never get down "
           "here, this should be handled internally by clip_read_packet())\n");
    ret = -1;
  }
  av_packet_unref(&pkt);
  return ret;
