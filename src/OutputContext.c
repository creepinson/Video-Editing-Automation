...
int set_muxer_params(OutputContext *oc, OutputStream *os, AVCodecContext *c) {
  ...
  if (ret < 0) {
    fprintf(
        stderr,
        "Could not copy the codec parameters to the output stream (muxer)\n");
    return ret;
  }
 ...
 * @param  oc           OutputContext
 * @param  op           OutputParameters for video and audio codecs/muxers
 * (filename as well)
...
int open_video_output(OutputContext *oc, OutputParameters *op, Sequence *seq) {
  ...
  if (ret < 0) {
    fprintf(
        stderr,
        "open_video_output(): Error occurred when opening output file: %s\n",
        av_err2str(ret));
 ...
 * @param  op       OutputParameters for video and audio codec/muxers (and
 * filename)
 * @return          >= 0 on success
 */
 ...
 * @return          >= 0 on success
 */
int set_output_params(OutputParameters *op, char *filename, VideoOutParams vp,
                      AudioOutParams ap) {
...
/**
 * Copy relevant codec context params (from decoder) into VideoOutParams struct
 * (for encoding) This can copy clip decoder settings to the encoder context
 * @param  op VideoOutParams
...
/**
 * Copy relevant codec context params (from decoder) into AudioOutParams struct
 * (for encoding)
 * @param  op AudioOutParams
 * @param  c  AVCodecContext containing data (ideally from decoder)
 */
