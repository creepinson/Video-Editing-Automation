...
int open_video_context(VideoContext *vid_ctx, char *filename) {
  ...
  int types[VIDEO_CONTEXT_STREAM_TYPES_LEN] = {AVMEDIA_TYPE_VIDEO,
                                               AVMEDIA_TYPE_AUDIO};
  ...
  if (!valid_rational(vid_ctx->video_time_base) ||
      !valid_rational(vid_ctx->audio_time_base)) {
    fprintf(stderr,
            "open_video_context() error: Invalid timebase for video[%d/%d] or "
            "audio [%d/%d]\n",
            vid_ctx->video_time_base.num, vid_ctx->video_time_base.den,
            vid_ctx->audio_time_base.num, vid_ctx->audio_time_base.den);
    return -1;
  }
  ...
  if (video_stream->duration <= 0 || video_stream->nb_frames <= 0) {
    ...
    if (!valid_rational(avg_fps)) {
      fprintf(stderr,
              "open_video_context() error: Invalid duration[%ld], "
              "nb_frames[%ld] and avg_frame_rate[%d/%d]\n",
              video_stream->duration, video_stream->nb_frames, avg_fps.num,
              avg_fps.den);
      return -1;
    }
...
int open_format_context(VideoContext *vid_ctx, char *filename) {
  if (vid_ctx->fmt_ctx) {
    fprintf(stderr,
            "open_format_context() error: Invalid params. vid_ctx->fmt_ctx "
            "must be NULL (initialized with init_video_context())\n");
    return -1;
  }
...
int open_codec_context(VideoContext *vid_ctx, enum AVMediaType type) {
  if (type != AVMEDIA_TYPE_VIDEO && type != AVMEDIA_TYPE_AUDIO) {
    fprintf(
        stderr,
        "Unsupported type '%s'. Video_Context does not support this stream\n",
 ...
 */
bool valid_rational(AVRational r) { return r.den > 0 && r.num > 0; }
