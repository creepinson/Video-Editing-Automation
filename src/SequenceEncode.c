...
int sequence_encode_frame(OutputContext *oc, Sequence *seq, AVPacket *pkt) {
  if (oc->video.done_flush && oc->audio.done_flush) {
    ...
    if (oc->last_encoder_frame_type == AVMEDIA_TYPE_VIDEO ||
        oc->video.flushing) {
      return seq_receive_enc_packet(oc, &(oc->video), seq, pkt);
    } else if (oc->last_encoder_frame_type == AVMEDIA_TYPE_AUDIO ||
               oc->audio.flushing) {
...
int seq_receive_enc_packet(OutputContext *oc, OutputStream *os, Sequence *seq,
  ...
  if (ret == 0) {
    ...
    // rescale packet timestamp values from codec_ctx(sequence) to output stream
    // timebase
    av_packet_rescale_ts(pkt, os->codec_ctx->time_base, os->stream->time_base);

    // printf("rescale %d/%d to %d/%d\n", seq->video_time_base.num,
    // seq->video_time_base.den, os->stream->time_base.num,
    // os->stream->time_base.den); rescale packet timestamp values from
    // sequence/codec_ctx to output stream timebase if(pkt->stream_index ==
    // oc->video.stream->index) {
    //     av_packet_rescale_ts(pkt, os->codec_ctx->time_base,
    //     os->stream->time_base);
    // } else if(pkt->stream_index == oc->audio.stream->index) {
    //     av_packet_rescale_ts(pkt, os->codec_ctx->time_base,
    //     os->stream->time_base);
    ...
    // output is not available in the current state - user must try to send
    // input
    return seq_send_frame_to_encoder(oc, seq, pkt);
    ...
    fprintf(stderr,
            "Legitmate encoding error when handling receive frame[%s]\n",
            av_err2str(ret));
    return ret;
  }
}
...
int seq_send_frame_to_encoder(OutputContext *oc, Sequence *seq, AVPacket *pkt) {
  ...
  if (ret < 0) {
    ...
    // if last clip(iterator will be reset to start), then flush video and audio
    // streams
    if (seq->clips_iter.current == seq->clips.head) {
      ...
      return sequence_encode_frame(oc, seq,
                                   pkt); // try to receive a flushed packet out
    } else {
      return ret;
    }
  }
  ...
  if (type == AVMEDIA_TYPE_VIDEO) {
    ...
    fprintf(stderr, "AVFrame type is invalid (must be AVMEDIA_TYPE_VIDEO or "
                    "AVMEDIA_TYPE_AUDIO)\n");
    return -1;
  }
}
...
int seq_handle_send_frame(OutputContext *oc, Sequence *seq,
  ...
  if (ret == 0) {
    ...
    return sequence_encode_frame(oc, seq,
                                 pkt); // receive the packet from encoder
  } else if (ret == AVERROR(EAGAIN)) {
