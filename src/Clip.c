 ...
 * Clip stores a reference to a video file and its data within an editing
 * sequence
 */
 ...
 * Returned Clip has its own Clip fields, but references the same VideoContext
 * as src. This is useful for using the same video file with multiple clips
...
int init_clip(Clip *clip, char *url) {
  ...
  if (clip->vid_ctx->url == NULL) {
    fprintf(
        stderr,
        "init_clip() error: Failed to allocate video_context filename[%s]\n",
        url);
    return -1;
  }
...
int open_clip(Clip *clip) {
  ...
  if (!(clip->vid_ctx->open)) {
    ...
    if ((ret = open_video_context(clip->vid_ctx, clip->vid_ctx->url)) < 0) {
      fprintf(stderr,
              "open_clip() error: Failed to open VideoContext for clip[%s]\n",
              clip->vid_ctx->url);
...
void close_clip(Clip *clip) { close_video_context(clip->vid_ctx); }
...
/**
    ...
    Return >= 0 on success
*/
int set_clip_bounds_pts(Clip *clip, int64_t orig_start_pts,
                        int64_t orig_end_pts) {
  int ret;
...
/**
    Seek to a clip frame, relative to the clip.
    @param Clip: clip to seek
    @param pts: relative pts to the clip (where zero represents
   clip->orig_start_pts)
*/
int seek_clip_pts(Clip *clip, int64_t pts) {
  ...
  if (pts < 0 || abs_pts > clip->orig_end_pts) {
    ...
    fprintf(stderr,
            "seek_clip_pts() error: seek pts[%ld] outside of clip bounds (0 - "
            "%ld)\n",
            pts, endBound);
    return -1;
  }
  ...
  if ((ret = seek_video_pts(clip->vid_ctx, abs_pts)) < 0) {
    fprintf(stderr,
            "seek_clip_pts() error: Failed to seek to pts[%ld] on clip[%s]\n",
  ...
  if ((ret = cov_video_pts(clip->vid_ctx, abs_pts)) < 0) {
    fprintf(stderr,
            "seek_clip_pts error: Failed to convert pts to frame index\n");
    return ret;
  }
 ...
 * @param  clip         Clip
 * @param  relative_pts pts relative to clip (where zero represents
 * clip->orig_start_pts)
...
/**
 * Convert absolute pts (from VideoContext), into a pts relative to the Clip
 * bounds
 * @param  clip    Clips
...
int64_t get_clip_end_frame_idx(Clip *clip) {
  return cov_video_pts(clip->vid_ctx,
                       clip->orig_end_pts - clip->orig_start_pts);
}
 ...
 * @return      true, if our clip has read the entire stream associated with
 * this packet. false, otherwise
 */
bool done_curr_pkt_stream(Clip *clip, AVPacket *pkt) {
  return (clip->done_reading_audio &&
          pkt->stream_index == clip->vid_ctx->audio_stream_idx) ||
         (clip->done_reading_video &&
          pkt->stream_index == clip->vid_ctx->video_stream_idx);
}

/**
    Read a single AVPacket from clip.
    Only returns packets if they are within clip boundaries (start_frame_pts -
   end_frame_pts) This is a wrapper on av_read_frame() from ffmpeg. It will
   increment the packet counter by 1 on each call. The idea is to call this
   function in a loop while return >= 0. When end of clip is reached the packet
   pointer will be reset to start of clip (seek 0)
    @param Clip to read packets
    @param pkt: output AVPacket when valid packet is read.. otherwise NULL
    Returns >= 0 on success. < 0 when reached EOF, end of clip boundary or
   error. End of clip only happens when both audio and video streams have
   reached their corresponding end_pts AVPacket is returned in param pkt. Return
   stream_idx when video or audio stream is complete.
*/
int clip_read_packet(Clip *clip, AVPacket *pkt) {
  ...
  } while (done_curr_pkt_stream(
      clip, &tmpPkt)); // Skip by packets from finished stream
  ...
  // If both audio and video streams have completed read cycle of the entire
  // clip
...
/**
 * Reset packet counter to beginning of clip and reset internal reading flags
 * (to allow another read cycle)
 * @param  clip Clip
 * @return      >= 0 on success
 */
 ...
 *                  0 if first->pts = second->pts.  (first and second at same
 * pts)
 */
...
int64_t compare_clips_sequential(Clip *f, Clip *s) {
  if (f == NULL || s == NULL) {
    fprintf(stderr,
            "compare_clips_sequential() error: params cannot be NULL\n");
    return -2;
  }
  double diff = difftime(f->vid_ctx->file_stats.st_mtime,
                         s->vid_ctx->file_stats.st_mtime);
  ...
  fprintf(
      stderr,
      "compare_clips_sequential() error: We should never get down here..\n");
  return -2;
}
 ...
 * This function handles clip positions (orig_start_pts and orig_end_pts) of
 * clips but not sequence positions (start_pts or end_pts) of clips. Sequence
 * positions are handled in cut_clip() in Sequence.c
 * @param  oc   Original Clip to be cut
 * @param  pts  pts relative to clip and clip timebase (zero represents
 * orig_start_pts) , where the highest value is the orig_end_pts
...
int cut_clip_internal(Clip *oc, int64_t pts, Clip **sc) {
  *sc = NULL;
  ...
  if ((pts < frame_duration) ||
      (pts >= (oc->orig_end_pts - oc->orig_start_pts))) {
    printf("cut_clip_internal(): pts out of range/cannot cut less than one "
           "frame.. ");
  ...
  *sc = copy_clip_vc(oc);
  ...
  if (ret < 0) {
    free_clip(sc);
    fprintf(stderr,
            "cut_clip_internal() error: Failed to set bounds on new clip\n");
    return ret;
  }
  return 0;
}
...
/**
 * Compare two clips within a linked list by date & time first, then
 * orig_start_pts second. This is useful for ordering clips by time they were
 * shot. Use insertSorted with this compare function to insert clips into a
...
int list_compare_clips_sequential(const void *first, const void *second) {
  if (first == NULL || second == NULL) {
    fprintf(stderr,
            "list_compare_clips_sequential() error: params cannot be NULL\n");
    return -2;
  }
  Clip *f = (Clip *)first;
  Clip *s = (Clip *)second;
