 ...
 * @param  fps          frames per second for the video stream. Video time base
 * = 1/(fps*1000) this fps is required and will base all functions that ask for
 *                      frameIndex.. ex: 30fps and frameIndex of 30 would be 1
 * second in.
 * @param  sample_rate  sample rate of the audio stream. Audio time base =
 * 1/sample_rate
 * @return              >= 0 on success
 */
...
/**
 * Initialize a new sequence, list of clips along with a custom compare function
 * (for insertSorted)
 ...
 * @return             >= 0 on success
 */
int init_sequence_cmp(Sequence *seq, double fps, int sample_rate,
...
/**
 * Allocate a clip within a sequence and use a reference to the same
 * videoContext for clips with the same url (filename)
 ...
 * This function will generate the sequence pts for the clip (clip->start_pts
 * and clip->end_pts)
 * @param  seq  Sequence
...
int sequence_insert_clip_sorted(Sequence *seq, Clip *clip) {
  ...
  if (node == NULL) {
    fprintf(stderr, "sequence_insert_clip_sorted() error: could not insert "
                    "clip in sorted order\n");
    return -1;
  }
  seq->clips_iter.current = seq->clips.head;
...
int sequence_ripple_delete_clip(Sequence *seq, Clip *clip) {
  ...
  if (curr == NULL) {
    fprintf(
        stderr,
        "sequence_delete_clip() error: clip data does not exist in sequence\n");
    return -1;
  }
  Node *next = curr->next;
  ...
  if (data == NULL) {
    fprintf(
        stderr,
        "sequence_delete_clip() error: Failed to delete clip from sequence\n");
    return -1;
  }
  if (next == NULL) {
...
/**
 * Iterate all sequence clips to find the clip that contains this frame_index in
 * sequence
 * @param  seq         Sequence
 ...
 *                      and clip timebase (where zero represents
 * clip->orig_start_pts)
 */
...
/**
 * Determine if sequence frame lies within a clip (assuming clip is within
 * sequence) Example: If this[xxx] is a clip where |---| is VideoContext:
 * |---[XXXXX]-----| then return of 0 would be the first X and return of 1 would
 * be the second X and so on.. We can use the successful return of this function
 * with seek_clip_pts() to seek within the clip!!
 * @param  sequence    Sequence containing clip
 * @param  clip        Clip within sequence
 * @param  frame_index index in sequence
 * @return             on success: pts relative to clip, and clip timebase
 * (where zero represents clip->orig_start_pts) on fail: < 0
 */
int64_t seq_frame_within_clip(Sequence *seq, Clip *clip, int frame_index) {
  ...
  int64_t pts_diff =
      seq_pts - clip->start_pts; // find pts relative to the clip!
 ...
 * @param  frame_index index in sequence, will be used to find clip and the clip
 * frame
 * @return             >= 0 on success
 */
 ...
 * This function uses clip_read_packet() and av_read_frame() internally. This
 * function works the exact same.. reading one packet at a time and incrementing
 * internally. (call this function in a loop while >= 0 to get full edit)
 * @param  seq Sequence containing clips
 * @param  pkt output AVPacket
 * @param close_clips when true, each clip will be closed at the end of its read
 * cycle (and reopened if read again) closing clips after usage will save memory
 * (RAM) but take more clock cycles. it takes roughly 10ms to open a clip. maybe
 * the most CPU efficient solution is opening the clips before usage of this
 * function (and keeping them open for a while). However, there is the concern
 * that opening many clips will use too much RAM (closing clips after usage
 * would solve this issue)
 * @return     >= 0 on success (returns packet.stream_index), < 0 when reached
...
void move_clip_pts(Sequence *seq, Clip *clip, int64_t start_pts) {
  ...
  // Automatically set the end_pts to the duration of the clip (which is set by
  // set_clip_bounds())
 ...
 * @param  seq          Sequence containing clip
 * @param  clip         Clip within sequence
 * @param  orig_pkt_ts  timestamp from AVPacket read directly from file (or
 * clip_read_packet())
 * @return              timestamp representation of the video packet in the
 * editing sequence!
 */
 ...
 * @param  orig_pkt_ts  timestamp from AVPacket read directly from file (or
 * clip_read_packet())
 * @return              timestamp representation of the audio packet in the
 * editing sequence!
 */
int64_t audio_pkt_to_seq_ts(Sequence *seq, Clip *clip, int64_t orig_pkt_ts) {
  ...
  int64_t audio_start_ts =
      av_rescale_q(clip->start_pts, seq->video_time_base, seq->audio_time_base);
  // add clip starting point in sequence
 ...
 */
void free_sequence(Sequence *seq) { clearList(&(seq->clips)); }
...
char *print_sequence(Sequence *seq) {
  ...
  for (int i = 0; currNode != NULL; i++) {
    ...
    sprintf(buf,
            "Clip [%d]\nurl: %s\nstart_pts: %ld\nend_pts: %ld\norig_start_pts: "
            "%ld\norig_end_pts: %ld\nvid_ctx: %p\n",
            i, c->vid_ctx->url, c->start_pts, c->end_pts, c->orig_start_pts,
            c->orig_end_pts, c->vid_ctx);
    catVars(&str, 1, buf);
    currNode = currNode->next;
  }
  return str;
    ...
    if (clip == NULL) {
      ...
      if (pkt.stream_index == clip->vid_ctx->video_stream_idx) {
        printf("Video packet! pts: %ld, dts: %ld, frame: %ld\n", pkt.pts,
               pkt.dts, cov_video_pts(clip->vid_ctx, pkt.pts));
