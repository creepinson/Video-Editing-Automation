 ...
 * test-resources/sequence/out7.mov 30 48000 test-resources/sequence/ 300 80 40
 */
int main(int argc, char **argv) {
  if (argc < 8) {
    printf("usage: %s output_file fps sample_rate source_dir duration "
           "cut_len_avg cut_len_var\n",
           argv[0]);
    printf("\nExplanation\n------------\n");
    printf(
        "output_file(string): output filename of encoded edit (ex. out.mov)\n");
    printf("fps(int): frames per second to use in sequence. All frame "
           "parameters are based on this (ex. 30 for 30fps)\n");
    printf("sample_rate(int): audio sample rate (ex. 48000 for 48kHz)\n");
    printf("source_dir(string): directory of raw video files that will be used "
           "in the edit\n");
    printf("duration(int): duration of output file (in frames - fps defined "
           "above)\n");
    printf("cut_len_avg(int): average length of cuts (in frames)\n");
    printf("cut_len_var(int): variability of average cuts used by the random "
           "number generator for max and min range\n");
    return -1;
  }
  ...
  init_sequence_cmp(&orig_seq, par.fps, par.sample_rate,
                    &list_compare_clips_sequential);
  init_sequence_cmp(&new_seq, par.fps, par.sample_rate,
                    &list_compare_clips_sequential);
...
int random_edit(Sequence *os, Sequence *ns, RandSpliceParams *par) {
  if (par->cut_len_var > par->cut_len_avg) {
    fprintf(stderr,
            "random_edit() error: cut_len_var[%d] must be less than "
            "cut_len_avg[%d]\n",
...
int cut_remove_insert(Sequence *os, Sequence *ns, int start_index,
  ...
  if (ret < 0) {
    fprintf(
        stderr,
        "cut_remove_insert() error: Failed to cut clip at start index[%d]\n",
        start_index);
    return ret;
  }
  ...
  if (ret < 0) {
    fprintf(stderr,
            "cut_remove_insert() error: Failed to cut clip at end index[%d]\n",
            end_index);
    return ret;
  }
  Clip *cut = NULL;
  ...
  if (!cut) {
    fprintf(stderr,
            "cut_remove_insert() error: Failed to find clip at cut center "
            "index[%d]\n",
            cut_center_index);
    return -1;
  }
  ...
  if (ret < 0) {
    ...
    fprintf(
        stderr,
        "cut_remove_insert() error: Failed to set clip bounds on cut copy\n");
    return ret;
  }
  ...
  if (ret < 0) {
    ...
    fprintf(
        stderr,
        "cut_remove_insert() error: failed to add clip[%s] to new sequence\n",
  ...
  if (ret < 0) {
    free_clip(&cut_copy);
    fprintf(stderr, "cut_remove_insert() error: Failed to delete clip from "
                    "original sequence\n");
    return ret;
  }
  return 0;
...
int pick_frames(Sequence *seq, RandSpliceParams *par, int *start_index,
  ...
  if (par->pick_frames_recur > PICK_FRAMES_RECUR_LIMIT) {
    fprintf(stderr,
            "pick_frames() error: par->pick_frames_recur[%d] > limit[%d]\nThis "
            "should never happen\n",
  ...
  if (!sc || !se) {
    fprintf(stderr,
            "pick_frames() error: clip does not exist at start[%d] or end[%d] "
            "index\n",
            s, e);
    return -1;
  }
