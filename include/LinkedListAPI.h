...
/**
 * Node of a linked list. This list is doubly linked, meaning that it has points
 *to both the node immediately in front of it, as well as the node immediately
 *behind it.
 **/
...
/** Function to initialize the list metadata head with the appropriate function
 *pointers.
 *@return the list struct
 *@param printFunction function pointer to print a single node of the list
 *@param deleteFunction function pointer to delete a single piece of data from
 *the list
 *@param compareFunction function pointer to compare two nodes of the list in
 *order to test for equality or order
 **/
List initializeList(char *(*printFunction)(void *toBePrinted),
                    void (*deleteFunction)(void *toBeDeleted),
                    int (*compareFunction)(const void *first,
                                           const void *second));
 ...
 *@pre data should be of same size of void pointer on the users machine to avoid
 *size conflicts. data must be valid. data must be cast to void pointer before
 *being added.
 *@post data is valid to be added to a linked list
 *@return On success returns a node that can be added to a linked list. On
 *failure, returns NULL.
 *@param data - is a void * pointer to any data type.  Data must be allocated on
 *the heap.
 **/
 ...
 *@pre 'List' type must exist and be used in order to keep track of the linked
 *list.
...
/** Clears the contents linked list, freeing all memory asspociated with these
 *contents. uses the supplied function pointer to release allocated memory for
 *the data
 *@pre 'List' type must exist and be used in order to keep track of the linked
 *list.
...
*@post The node to be added will be placed immediately before or after the first
occurrence of a related node
*@param list a pointer to the dummy head of the list containing function
pointers for delete and compare, as well as a pointer to the first and last
element of the list.
*@param toBeAdded a pointer to data that is to be added to the linked list
**/
 ...
 *@post If toBeDeleted was found, the node associated with it is removed from
 *the list and freed. The list is re-linked. Otherwise the List is unchanged.
 *@param list pointer to the dummy head of the list containing deleteFunction
 *function pointer
...
/**Returns a pointer to the data at the front of the list. Does not alter list
 *structure.
...
/**Returns a pointer to the data at the back of the list. Does not alter list
 *structure.
 *@pre The list exists and has memory allocated to it
 *@param the list struct
...
the list traversed from  head to tail. Utilize the list's printData function
pointer to create the string. returned string must be freed by the calling
function.
 *@pre List must exist, but does not have to have elements.
 *@param list Pointer to linked list dummy head.
 *@return on success: char * to string representation of list (must be freed
after use).  on failure: NULL
 **/
 ...
 *@post List remains unchanged.  The iterator has been allocated and points to
 ...
 * This function returns the data at head of the list the first time it is
 *called after. the iterator was created. Every subsequent call returns the data
 *associated with the next element. Returns NULL once the end of the iterator is
 *reached.
 *@pre List exists and is valid.  Iterator exists and is valid.
 *@post List remains unchanged.  The iterator points to the next element on the
 *list.
 *@return The data associated with the list element that the iterator pointed to
 ...
 * @param  iter  ListIterator containing current node (where seek node will be
 * saved)
 ...
 *@return on success: number of eleemnts in the list (0 or more).  on failure:
 *-1 (e.g. list not initlized correctly)
 **/
int getLength(List list);

/** Function that searches for an element in the list using a comparator
 *function. If an element is found, a pointer to the data of that element is
 *returned Returns NULL if the element is not found.
 *@pre List exists and is valid.  Comparator function has been provided.
 *@post List remains unchanged.
 *@return The data associated with the list element that matches the search
 *criteria.  If element is not found, return NULL.
 *@param list - a list sruct
 *@param customCompare - a pointer to comparator fuction for customizing the
 *search
 *@param searchRecord - a pointer to search data, which contains seach criteria
 *Note: while the arguments of compare() and searchRecord are all void, it is
 *assumed that records they point to are all of the same type - just like
 *arguments to the compare() function in the List struct
 **/
